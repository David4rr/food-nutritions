...
class AnalyticsEngine {
  static AnalyticsData compute(List<ProductHistory> items, double tCal, double tPro) {
    ...
    for (final item in items) {
      if (startOfToday.difference(item.scanDate).inDays <= 14) {
        final key = '${item.scanDate.year}-${item.scanDate.month.toString().padLeft(2, '0')}-${item.scanDate.day.toString().padLeft(2, '0')}';
    ...
    for (var i = 6; i >= 0; i--) {
      ...
      final keyLast = '${dateLast.year}-${dateLast.month.toString().padLeft(2, '0')}-${dateLast.day.toString().padLeft(2, '0')}';
      lastWeek.add(_computeDayEntry(dateLast, recentItems[keyLast] ?? []));
    }

    final totalCalThisWeek = thisWeek.fold<double>(0, (sum, e) => sum + e.calories);
    final totalCalLastWeek = lastWeek.fold<double>(0, (sum, e) => sum + e.calories);
    ...
    final sortedDays = List<DayEntry>.from(thisWeek)..sort((a, b) {
    ...
    final bestDay = sortedDays.first.calories > 0 ? weekdays[sortedDays.first.date.weekday - 1] : '-';
    final worstDay = sortedDays.last.calories > 0 ? weekdays[sortedDays.last.date.weekday - 1] : '-';

    final insights = <String>[];
    if (avgPro < tPro) {
      insights.add('Protein mingguan rata-rata di bawah target (${tPro.toStringAsFixed(0)}g). Tambah lauk tinggi protein.');
    } else {
      insights.add('Asupan protein sangat baik dan konsisten minggu ini! Pertahankan.');
    }
    
    if (trend > 10) {
      insights.add('Terjadi lonjakan kalori sebesar ${trend.toStringAsFixed(1)}% dibanding minggu lalu.');
    } else if (trend < -10) {
      insights.add('Defisit kalori cukup signifikan (${trend.toStringAsFixed(1)}%) dari minggu sebelumnya.');
    } else {
      insights.add('Asupan kalori kamu cukup stabil jika dibandingkan dengan minggu sebelumnya.');
    }
  ...
  static DayEntry _computeDayEntry(DateTime date, List<ProductHistory> dailyItems) {
    double c = 0, p = 0, f = 0, carbs = 0;
    for (final item in dailyItems) {
      final ratio = (item.estimatedCaloriesPerProduct != null && item.calories > 0) 
