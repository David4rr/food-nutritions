...
class AnalyticsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<Box<dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        final tPro = (box.get('target_protein_min') as num?)?.toDouble() ?? 50.0;
              ...
              _buildNativeCard(
                ...
                child: Column(
                  children: [
                    Row(
                      children: [
                        Expanded(child: _buildMetric('Kalori Rata-rata', '${data.avgCalories.toStringAsFixed(0)} kkal')),
                        Expanded(child: _buildMetric('Protein Rata-rata', '${data.avgProtein.toStringAsFixed(0)} g')),
                      ],
                    ),
                  ...
                  children: [
                    Expanded(
                      ...
                      child: Column(
                        ...
                        children: [
                          _buildLegend(Theme.of(context).colorScheme.secondary, 'Protein', '${(data.macroProtein * 100).toStringAsFixed(1)}%'),
                          const SizedBox(height: 8),
                          _buildLegend(Theme.of(context).primaryColor, 'Karbo', '${(data.macroCarbs * 100).toStringAsFixed(1)}%'),
                          const SizedBox(height: 8),
                  ...
                  children: [
                    ...
                    ...data.insights.map((insight) => Padding(
                      ...
                      child: Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('✦ ', style: TextStyle(fontSize: 16, color: Color(0xFFE45BA5), fontWeight: FontWeight.bold)),
                          Expanded(
                            child: Text(
                              insight,
  ...
  Widget _buildNativeCard({required String title, String? subtitle, required Widget child}) {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(
        color: Colors.white,
